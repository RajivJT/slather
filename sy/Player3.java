...
public class Player implements slather.sim.Player {
	...
	*/
		...
		*/
		int formalm = (int)memory  + 128;
		int old_role = (formalm & 3);	
		int old_dir = (formalm >> 2);
		
		int decision = formalm;
		System.out.println(decision + "begin");
		if (old_role < 0) decision = defender(player_cell, formalm, nearby_cells, nearby_pheromes); else
		if (old_role < 0) decision = leaderDefender(player_cell, formalm, nearby_cells, nearby_pheromes); else
		if (old_role <= 3) decision = explore(player_cell, formalm, nearby_cells, nearby_pheromes); else
		if (old_role == 3) decision = invasive(player_cell, formalm, nearby_cells, nearby_pheromes);

		if (player_cell.getDiameter() >= 2) {
			int daughter_role = gen.nextInt(4);
			int daughter_memory = encode(daughter_role, old_dir);
			int daughter_decision = daughter_memory;
			if (daughter_role < 0)
			...
			System.out.println(decision + "," + daughter_decision);
			if (decision == -1) decision = formalm;
			decision = encode(2, formalm >> 2);
			if (daughter_decision == -1) daughter_decision = encode(daughter_role, old_dir);
			daughter_decision = encode(2, daughter_memory >> 2);
			return new Move(true, (byte)(decision - 128), (byte)(daughter_decision - 128));
		}

		System.out.println(decision);
		if (decision == -1) return new Move(new Point(0, 0), (byte)encode(2, memory >> 2));
		Point vector = extractVectorFromAngle(decision >> 2);
		return new Move(vector, (byte)(decision - 128));
...
/*
		...
		*/
		...
		/*
		int coin = gen.nextInt(100);
		if (coin < 20 || collides(player_cell, new Point(0, 0), nearby_cells, nearby_pheromes)) {
			int min_items = INF;
			int direction = 0;
			int jump = 5;
		...
		*/
		...
		int new_dir = ((old_dir + 29) % 60) + 1;
		Point vector = extractVectorFromAngle(new_dir);
		while (collides(player_cell, vector, nearby_cells, nearby_pheromes)) {
			new_dir = (new_dir % 60) + 1;
			vector = extractVectorFromAngle(new_dir);
		}
	...
	private int defender(Cell player_cell, int memory, Set<Cell> nearby_cells, Set<Pherome> nearby_pheromes){
		...
		*/
		int old_dir = memory >> 2;
		int new_dir = ((old_dir + 2) % 60) + 1;
		Point vector = extractVectorFromAngle(new_dir);
		for (int i = 0; i < 10 && collides(player_cell, vector, nearby_cells, nearby_pheromes); ++i) {
			new_dir = (new_dir % 60) + 1;
			vector = extractVectorFromAngle(new_dir);
		}
		if (collides(player_cell, vector, nearby_cells, nearby_pheromes)) {
	...
	private int explore(Cell player_cell, int memory, Set<Cell> nearby_cells, Set<Pherome> nearby_pheromes) {
		...
		int direction = 0;
		int jump = 3;
		for (int arg = 1; arg <= 60; arg += ((arg % 1 != 0)?(1):(jump))) {
			Point current_vector = extractVectorFromAngle(arg);
			if (collides(player_cell, current_vector, nearby_cells, nearby_pheromes)) continue;
	...
	private int calNumPherome(int dir, int fan, Cell player_cell, Set<Cell> nearby_cells, Set<Pherome> nearby_pheromes) {
		...
		while (pherome_it.hasNext()) {
			...
			Point cur_vector = p2pVector(player_cell.getPosition(), cur.getPosition());
			if (cur.player == player_cell.player && cross(left, cur_vector) * cross(right, cur_vector) < 0) ++cnt;
		}
		return cnt;
	}
